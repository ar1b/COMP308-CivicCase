...
function NotificationBellInner() {
  ...
  const handleMarkRead = async (id) => { await markRead({ variables: { id } }); refetch() }
  const handleMarkAll  = async ()     => { await markAllRead(); refetch() }
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(v => !v)} className="relative p-1 hover:bg-blue-600 rounded-lg transition-colors">
...
export default function NotificationBell() {
  return <ApolloProvider client={client}><NotificationBellInner /></ApolloProvider>
}
